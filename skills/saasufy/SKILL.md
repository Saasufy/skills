...
[authentication.md](authentication.md): Explains how users authenticate with a Saasufy service, how to add and configure OAuth providers via the `OAuthProvider` model, and how to support email signups using the shared Saasufy Keycloak instance, as well as how to run a provider in multiple environments such as `keycloak:dev`.
